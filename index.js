...
function createIPObject(ipArrayList) {
        ...
        for (let i = 0; i < ipArrayList.length; i++) {
                ...
                // if ip address does not contain cidr value push the same ip start/end value to ip array object
                        ...
                        if (entry.includes(':')) { // ipv6 check
                                let i6StartEnd = ipv6.findIPv6Block(entry + '/128')
                                ipv6ObjectArray.push(new util.IPStartEndBlock(ipv6.v6ToInteger(i6StartEnd[0]), ipv6.v6ToInteger(i6StartEnd[1])));
                        } else {
                                ipv4ObjectArray.push(new util.IPStartEndBlock(ipv4.ipToIntegerConversion(entry), ipv4.ipToIntegerConversion(entry)));
                        }
                }
        }
        return {ipv4ObjectArray, ipv6ObjectArray};
}

function banCheck(options) {
        const ipArray = loadIPList(options.source);
        const {ipv4ObjectArray, ipv6ObjectArray} = createIPObject(ipArray);
        ...
        return function(req, res, next) {
                ...
                if (ip.includes(':')) { // ipv6 check
                        ...
                        const isv6Banned = ipv6ObjectArray.some(block => {
                                return convertedIP >= block.start && convertedIP <= block.end
                        });
